...
def add_movie (movie_name, year):
    ...
    tmdb_id = get_tmdb_id(movie_name, year)
    ...
    if(tmdb_id != c.ERROR):#movie search was successful
        ...
        if(tmdb_json != c.ERROR and omdb_json != c.ERROR):
            movie.set_properties(tmdb_json, omdb_json)
            return movie
        else:
            return c.ERROR
    else:
...
def get_movies_in_dir(path):
    ...
    for entry in os.scandir(path):
        if os.path.isdir(entry) or entry.name.lower().endswith('.mkv'):#a movie
            entry_name = entry.name
            #is a movie file but has no respective directory
            #creates dir and moves file inside it
            if entry.name.lower().endswith('.mkv'):
                #trim entry_name from .mkv extension
                entry_name = entry_name[:-4]
                change_file_into_dir(path, entry)

            #get current movie
            movie_year = get_name_and_year(entry_name)
            curr_movie = add_movie(movie_year[0], int(movie_year[1]))

            if(curr_movie != c.ERROR):#movie was found
